...
{
	public class AntiGravBeam : ModProjectile
	{
		...
		public override void AI()
		{
			...
			switch (mode)
			{
				case MODE_NPC:
					UpdateAltPoints();
					target.velocity.X += (nx - px) / 200.0f;
					target.velocity.Y += (ny - py) / 200.0f;
				...
				case MODE_PLAYER:
					...
					{
						UpdatePlayerPoints();
						ptarget.velocity.X += (nx - px) / 200.0f;
						ptarget.velocity.Y += (ny - py) / 200.0f;
				...
				case MODE_GROUND:
					...
					{
						UpdatePoints();
						owner.velocity.X += (nx - ox) / 300.0f;
						owner.velocity.Y += (ny - oy) / 300.0f;
		...
		private void DrawBit(SpriteBatch s, float x, float y, float ang = 0.0f, float scale = 1.0f, float fade = 1.0f)
		{
			...
			s.Draw(t, new Vector2(x - Main.screenPosition.X, y - Main.screenPosition.Y),
									 ...
									 new Color((int)(scale * 100.0f), 0xA0, (int)(fade * 150.0f * ts)), ang, new Vector2(t.Width >> 1, t.Height >> 1), scale, SpriteEffects.None, 0f);
		}
