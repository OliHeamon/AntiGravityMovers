...
{
	public class WeaponizedAntiGravBeam : ModProjectile
	{
		...
		public override void AI()
		{
			...
			{
				case MODE_NPC:
					if (isOwner && Main.mouseRight && (!pulsing) && (!target.friendly))
					{
						pulsing = true;
						pulse = 0.0f;
					}
				...
				case MODE_PLAYER:
					...
					if (Main.netMode != NetmodeID.Server)
					{
						if (isOwner && Main.mouseRight && (!pulsing))
						{
							pulsing = true;
							pulse = 0.0f;
						}
		...
		private void DrawBit(SpriteBatch s, float x, float y, float ang = 0.0f, float scale = 1.0f, float fade = 1.0f)
		{
			...
			s.Draw(t, new Vector2(x - Main.screenPosition.X, y - Main.screenPosition.Y),
									 ...
									 new Color((int)(fade * 150.0f * ts), 0, 0), ang, new Vector2(t.Width >> 1, t.Height >> 1), scale, SpriteEffects.None, 0f);
		}

		private float GetScale2(float x, float d)
		{
			return Math.Max(0.0f, -1.0f * ((x - d) * (x - d) * 20.0f) + 1.0f);
		}
		...
		public override void PostDraw(SpriteBatch spriteBatch, Color lightColor)
		{
			...
			for (float i = 0.0f; i < cap; i += 1.0f)
			{
				...
				DrawBit(spriteBatch, dx, dy, aRot + prec * 8.0f, GetScale(prec * 2.0f) + ((pulsing) ? GetScale2(prec, pulse) : 0.0f), (float)Math.Sin((aRot * 3.0f + prec * 4.0f)) / 4.0f + 0.75f);
			}
		}
